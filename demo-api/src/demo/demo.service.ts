...
export class DemoService {
  getProtectedData(user: any) {
    return {
      ...
      user: { id: user.userId, email: user.email, role: user.role, authMethod: user.authMethod },
      data: {
  ...
  getAppUserData(user: any) {
    return {
      ...
      timestamp: new Date().toISOString(),
      user: {
        id: user.userId, email: user.email, full_name: user.full_name,
        role: user.role, appRole: user.appRole, authMethod: user.authMethod,
      },
      data: {
  ...
  configureAppAdmin(user: any, body: any) {
    return {
      ...
      timestamp: new Date().toISOString(),
      user: { id: user.userId, email: user.email, appRole: user.appRole },
      configured: body,
      verifiedBy: 'AppAdminGuard',
    };
  }
