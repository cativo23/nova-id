...
export async function expandPermission(namespace, object, relation, maxDepth = 3) {
  try {
    const url = `${ketoReadUrl}/relation-tuples/expand?namespace=${namespace}&object=${object}&relation=${relation}&max-depth=${maxDepth}`
    const response = await fetch(url, {
      method: 'GET',
      credentials: 'include',
      mode: 'cors'
...
export async function getNamespaceRelations(namespace) {
  try {
    const url = `${ketoReadUrl}/relation-tuples?namespace=${encodeURIComponent(namespace)}`
    const response = await fetch(url, {
      method: 'GET',
      credentials: 'include',
      mode: 'cors'
...
export async function getUserRole(userId) {
  try {
    ...
    const roleRelation = relations.find(r =>
      r.namespace === 'ranks' && r.relation === 'member'
    )
